 ...
 * SELECTIVO: Solo aplica a elementos que NO estén dentro de modales o componentes críticos
 */
window.aplicarEstilosNeomorficos = function() {
    // NO aplicar automáticamente a .card dentro de modales, sidebars, topbars
    // para evitar romper el layout. Solo aplicar en el contenido principal.
    const mainContent = document.getElementById('main-content');
    if (!mainContent) return;
    
    // Aplicar a cards del contenido principal que no tengan la clase ya
    mainContent.querySelectorAll('.card:not(.neomorphic):not([id="parcelModal"]):not(.modal-content)').forEach(card => {
        // Solo si el card no está dentro de un modal
        if (!card.closest('.modal')) {
            card.classList.add('neomorphic');
        }
    });
    
    // Aplicar a inputs del contenido principal
    mainContent.querySelectorAll('.form-control:not(.neomorphic):not(.form-control-sm)').forEach(input => {
        if (!input.closest('.modal') && !input.closest('#parcelModal')) {
            input.classList.add('neomorphic');
        }
    });
    
    // NO aplicar automáticamente a botones (pueden romper tamaños)
    // Los botones se estilean via el CSS override
};
